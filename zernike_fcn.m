function z = zernike_fcn(p,r,theta,nflag)
%zernike_fcn Single-index Zernike functions on the unit circle.
%   Z = zernike_fcn(P,R,THETA) returns the Pth Zernike functions evaluated
...
%
%   Z = zernike_fcn(P,R,THETA,'norm') returns the normalized Zernike
...
%   for all p.
%
%   NOTE: zernike_fcn returns the same output as ZERNFUN, for the first 36
...
%       p = 0:15;
%       z = nan(size(X));
%       y = zernike_fcn(p,r(idx),theta(idx));
...
% -----------------------------
if min(size(p))~=1
    error('zernike_fcn:Pvector','Input P must be vector.')
end
if any(p)>35
    error('zernike_fcn:P36', ...
        ['zernike_fcn only computes the first 36 Zernike functions ' ...
        '(P = 0 to 35).'])
end
...
switch nargin
    ...
    otherwise
        error('zernike_fcn:nargin','Incorrect number of inputs.')
end
% EOF zernike_fcn
end
